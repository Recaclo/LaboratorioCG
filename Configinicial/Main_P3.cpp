...
int main() {
    ...
    // POSICIONES DE LOS 5 CUBOS (centrados y simetricos)
    // Separacion uniforme para que se vean "bonitos"
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glClearColor(0.95f, 0.95f, 0.95f, 1.0f); // fondo gris claro para que se vea mas "bonito"
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        // mandamos la inofrmacion a la parte del shader
        ourShader.Use();

        // Se crean matrices view (camara) y se manda projection una vez
        glm::mat4 view = glm::mat4(1.0f);

        // Colocamos la camara un poco alejada para ver los 5 cubos completos
        ...
        // Dibujamos 5 cubos estaticos 
        for (int i = 0; i < 5; i++)
        {
            // Matriz model para cada cubo
            glm::mat4 model = glm::mat4(1.0f);

            // 1) Mover cada cubo a su posicion
            model = glm::translate(model, cubePositions[i]);

            // 2) Pequeña inclinacion fija para que se vea 3D (NO es animacion)
            model = glm::rotate(model, glm::radians(25.0f), glm::vec3(0.0f, 1.0f, 0.0f));
            model = glm::rotate(model, glm::radians(12.0f), glm::vec3(1.0f, 0.0f, 0.0f));

            // 3) Escala uniforme para que todos sean simetricos (no deformados)
            model = glm::scale(model, glm::vec3(1.0f));

            // Mandamos la matriz model y dibujamos el cubo
